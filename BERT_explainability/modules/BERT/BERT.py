...
class BertSelfAttention(RelProp):
    ...
    def forward(
        ...
        context_layer = self.matmul2([attention_probs, value_layer])  # value_layer: V, use this to calculate the embedding of each attention layer
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = Linear(config.hidden_size, config.hidden_size) # V in QKV
        self.LayerNorm = LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = Dropout(config.hidden_dropout_prob)
        self.add = Add()
